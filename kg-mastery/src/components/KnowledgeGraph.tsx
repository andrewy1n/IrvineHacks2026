...
export function KnowledgeGraph() {
  ...
  const activeConceptId = selectedNode?.id;
  ...
  // Track if graphData has structurally changed (vs just mastery updates)
  const edgesString = graphData ? JSON.stringify(graphData.links) : "[]";

  // Initialize data and compute topological levels for left-to-right layout
  useEffect(() => {
    if (!graphData || graphData.nodes.length === 0) return;

    const nodesCopy: SimNode[] = graphData.nodes.map((n) => {
    ...
    const linksCopy: SimLink[] = graphData.links
  ...
  useEffect(() => {
    ...
    const simulation = d3
      ...
      .force(
        "collide",
        d3.forceCollide<SimNode>().radius((d) => getNodeRadius(d.label, d.relevance || 0.6) * 1.5),
      )
      .force(
        "x",
  ...
  }, [bounds.width, bounds.height, simNodes.length, simLinks]);
  ...
  return (
    <div
      className="relative w-full h-full overflow-hidden bg-gradient-to-br from-[#fafafa] via-white to-gray-50/50 font-sans"
      onWheel={handleWheel}
    ...
    >
      ...
      <div ref={containerRef} className="w-full h-full">
        ...
        <svg className="absolute inset-0 pointer-events-none" width={bounds.width || "100%"} height={bounds.height || "100%"} style={{ overflow: "visible" }}>
          ...
          <g>
            {simLinks.map((link, i) => {
              ...
              const isPath = false; // Add path highlighting logic if desired
              ...
              return (
                <g key={`link-${i}`}>
                  <path
                    ...
                    opacity={edgeRevealed ? (isPath ? 1 : 0.4) : 0}
                    fill="none"
        ...
        <div className="absolute inset-0 pointer-events-none" data-graph-nodes>
          {simNodes.map((node) => {
            ...
            const targetOpacity = nodeRevealed ? 1 : 0;
            const targetScale = nodeRevealed ? (isActive ? 1.15 : 1) : 0;
      ...
      {/* Legend */}
      <div className="absolute bottom-4 left-4 p-4 rounded-xl bg-white/70 border border-gray-200/80 backdrop-blur-md shadow-sm">
