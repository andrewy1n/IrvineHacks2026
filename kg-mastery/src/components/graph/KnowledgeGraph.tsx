...
/** Edges with source/target as string id or object with id (e.g. GraphLink from API). */
export type GraphEdgeInput = Array<{
...
function getEdgeSourceId(e: GraphEdgeInput[number]): string {
  return typeof e.source === "object" ? e.source.id : e.source;
}
function getEdgeTargetId(e: GraphEdgeInput[number]): string {
...
export default function KnowledgeGraph({
  ...
  useEffect(() => {
    ...
    const linksCopy: SimLink[] = edges
      .filter((e) => nodeIds.has(getEdgeSourceId(e)) && nodeIds.has(getEdgeTargetId(e)))
      .map((e) => ({ source: getEdgeSourceId(e), target: getEdgeTargetId(e) }));
  ...
  useEffect(() => {
    ...
    const simulation = d3
      ...
      .force(
        "link",
        d3.forceLink<SimNode, SimLink>(simLinks).id((d: SimNode) => d.id).distance(180)
      )
      .force("charge", d3.forceManyBody().strength(-500))
      .force(
        "collide",
        d3.forceCollide<SimNode>().radius((d: SimNode) => getNodeRadius(d.label, d.relevance ?? 0.6) * 2)
      )
      .force("x", d3.forceX<SimNode>().x((d: SimNode) => getTargetX(d)).strength(0.8))
    ...
    if (xs.length > 0 && ys.length > 0) {
      ...
      const scaleX = bounds.width / (maxX - minX);
      const scaleY = bounds.height / (maxY - minY);
  ...
  return (
    <div
      className="relative w-full h-full overflow-hidden rounded-xl border font-sans"
      style={{
    ...
    >
          ...
          >
            <defs>
              ...
              <marker
                ...
                markerUnits="userSpaceOnUse"
              >
                <polygon points="0 0, 14 5, 0 10" style={{ fill: "var(--gold)" }} />
              </marker>
            </defs>
            <g>
              {simLinks.map((link, i) => {
                ...
                const d = `M${sx},${sy} C${sx + cdx / 2},${sy} ${tx - cdx / 2},${ty} ${tx},${ty}`;
                                ...
                                const particleColor = isPath ? "var(--gold)" : "#64748b";
                const strokeColor = isPath ? "var(--gold)" : "#64748b";
                return (
                  <g key={`link-${i}`}>
                    <path
                      d={d}
                    ...
                    {edgeRevealed && (
                      <motion.circle
                        ...
                        style={{ offsetPath: `path("${d}")`, fill: particleColor } as React.CSSProperties}
                      />
                    )}
                  </g>
                );
          ...
          <div className="absolute inset-0 pointer-events-none" data-graph-nodes>
            {simNodes.map((node) => {
              ...
              const isActive = node.id === activeConceptId;
                            const glowColorHex = isActive ? "#ffffff" : borderColor;
              ...
              return (
                <motion.div
                  ...
                  style={{
                    ...
                    background:
                      isInSet || isActive
                        ? `linear-gradient(135deg, ${glowColorHex}20, ${glowColorHex}10)`
                        : baseFill,
                    border: isInSet
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center p-1.5 text-center pointer-events-none overflow-hidden gap-1">
                    <p
                      ...
                      style={{
                        ...
                        wordBreak: "keep-all",
                        overflowWrap: "normal",
                        color: isInSet || isActive ? "var(--gold)" : "var(--foreground)",
                      }}
                    >
                      {node.label}
                    </p>
      ...
      <div
        className="absolute bottom-4 right-4 flex flex-col gap-2 flex-shrink-0"
        style={{ color: "var(--gold)" }}
      >
        <button
          type="button"
          onClick={() => setZoom((z) => Math.min(MAX_ZOOM, z * 1.25))}
          className="min-w-11 min-h-11 w-11 h-11 flex items-center justify-center p-2 rounded-lg border backdrop-blur-md transition-colors hover:opacity-100 shrink-0"
          style={{
            background: "var(--card)",
            borderColor: "var(--card-border)",
          }}
      ...
      <div
        className="absolute bottom-4 left-4 min-w-[200px] p-4 rounded-xl border backdrop-blur-md flex-shrink-0"
        style={{
